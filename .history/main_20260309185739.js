...
function createChatWidget() {
    ...
    function extractTextDelta(payload, eventName) {
        ...
        if (payload.type && typeof payload.type === 'string') {
            if (payload.type.includes('delta') && typeof payload.delta === 'string') {
                return payload.delta;
            }
            if (payload.type.includes('text') && typeof payload.text === 'string') {
                return payload.text;
            }
        }
    ...
    async function streamResponseToElement(response, messageEl, onChunk) {
        if (!response.body) {
            ...
            if (text) {
                messageEl.innerHTML = renderMarkdownStreaming(text);
            }
            return;
        }
        ...
        const parser = createSseParser(({ event, data }) => {
            ...
            if (deltaText) {
                fullText += deltaText;
                messageEl.innerHTML = renderMarkdownStreaming(fullText);
        ...
        if (!hasStarted && !streamErrored && !fullText) {
            const fallbackText = '';
            if (fallbackText) {
                messageEl.innerHTML = renderMarkdownStreaming(fallbackText);
            }
        }
    }
