...
function createChatWidget() {
    ...
    function renderMarkdown(rawText) {
        ...
        let text = rawText.replace(/```([^\n]*)\n([\s\S]*?)```/g, (match, lang, code) => {
            ...
            const safeLang = lang ? escapeHtml(lang.trim()) : '';
            const className = safeLang ? ` class="language-${safeLang}"` : '';
            const html = `<pre><code${className}>${safeCode}</code></pre>`;
        ...
        text = text.replace(/\[([^\]]+)\]\(([^)\s]+)\)/g, (match, label, url) => {
            ...
            const html = `<a href="${safeUrl}" target="_blank" rel="noopener noreferrer">${safeLabel}</a>`;
            links.push(html);
        ...
        text = text.replace(/\u0000CODEBLOCK(\d+)\u0000/g, (match, idx) => codeBlocks[Number(idx)] || '');
        text = text.replace(/\u0000LINK(\d+)\u0000/g, (match, idx) => links[Number(idx)] || '');

        text = text.replace(/`([^`]+)`/g, (match, code) => `<code>${code}</code>`);
        text = text.replace(/\*\*([^*]+)\*\*/g, (match, bold) => `<strong>${bold}</strong>`);
        text = text.replace(/__([^_]+)__/g, (match, bold) => `<strong>${bold}</strong>`);
        text = text.replace(/\*([^*]+)\*/g, (match, italic) => `<em>${italic}</em>`);
        text = text.replace(/_([^_]+)_/g, (match, italic) => `<em>${italic}</em>`);
        text = text.replace(/\n/g, ' ');
    ...
    function parseSseLines(buffer, onData) {
        ...
        while (true) {
            ...
            if (lineEnd === -1) break;
            let line = remaining.slice(0, lineEnd);
    ...
    async function streamResponseToElement(response, messageEl, onChunk) {
        ...
        function tick() {
            ...
            const charCount = Math.min(3, pending.length);
            const slice = pending.slice(0, charCount);
            pending = pending.slice(charCount);
            accumulated += slice;
            messageEl.innerHTML = renderMarkdown(accumulated);
            scrollToBottom();
        ...
        while (true) {
            ...
            if (chunk) {
                ...
                sseBuffer += chunk;
                const parsed = parseSseLines(sseBuffer, (data) => {
                    if (DEBUG_SSE) {
                        console.log('SSE data event:', JSON.stringify(data));
                    }
                    ...
                    if (skipConnected && normalized.toLowerCase() === 'connected') {
                        return;
                    }
                    skipConnected = false;
                    pending += data;
                ...
                sseEventData = parsed.eventData;
                if (doneReceived) break;
            }
        }

        if (sseEventData) {
            const normalized = sseEventData.trim();
            if (normalized && normalized !== '[DONE]' && normalized !== 'DONE') {
                if (!(skipConnected && normalized.toLowerCase() === 'connected')) {
                    skipConnected = false;
                    pending += sseEventData;
                    if (onChunk) onChunk(sseEventData);
                    if (!isAnimating) {
                        isAnimating = true;
                        requestAnimationFrame(tick);
                    }
        ...
        if (pending) {
            await new Promise((resolve) => {
                const finalize = () => {
                    if (!isAnimating && !pending) return resolve();
            ...
            try {
                ...
                await streamResponseToElement(res, botMessageEl, () => {
                    ...
                    if (!firstChunkAt) {
                        ...
                        console.log(`Streaming: primer chunk en ${delta}ms`);
                    }
                    if (!hasStarted) {
                        hasStarted = true;
                ...
                console.log(`Streaming: ${totalChunks} chunks en ${totalMs}ms`);
                scrollToBottom();
            } catch (err) {
                clearTimeout(typingTimer);
                removeTypingIndicator();
